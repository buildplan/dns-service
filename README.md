...
**Option 2 (Standard Node):** Switch to the official Node image by changing this line in `Dockerfile`:

```bash
# Change from:
FROM dhi.io/node:24.x

# To:
FROM node:24-alpine
```

Note: You'll also need to add back user permissions and dumb-init if using the standard image.
